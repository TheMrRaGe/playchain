...
BASE_SCRIPTS = [
    ...
    # Playchain-specific tests
    'playchain_btxid.py',
    'playchain_create_burns.py',
    'playchain_dualalgo.py',
    'playchain_gameblocks.py',
    'playchain_gamepending.py',
    '_postico_fork.py',
    'playchain_premine.py',
    'playchain_trackedgames.py',
    'playchain_trading.py',
]
...
SKIPPED = [
    ...
    # are active from the start in Playchain.
    'feature_dersig.py',
    'feature_cltv.py',
]
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    try:
        ...
        if subprocess.run(["pgrep", "-x", "playchaind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a playchaind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
def check_script_prefixes():
    ...
    good_prefixes_re = re.compile("^(example|feature|interface|mempool|mining|p2p|rpc|wallet|tool|auxpow|name|playchain)_")
