...
class ScanblocksTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        wallet.send_to(from_node=node,
                       scriptPubKey=bytes.fromhex(node.validateaddress("cVfBe2wqDMmJquQLhvp87idMywbNtbnYVb")['scriptPubKey']),
                       amount=1 * COIN)
        ...
        # FIXME: Re-enable with modified false-positive for Playchain
        if False:
