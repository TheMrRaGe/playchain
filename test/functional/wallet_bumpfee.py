...
class BumpFeeTest(BitcoinTestFramework):
    ...
    def test_invalid_parameters(self, rbf_node, peer_node, dest_address):
        ...
        for mode in ["foo", Decimal("3.1415"), "sat/B", "BITS/kB"]:
...
def test_notmine_bumpfee(self, rbf_node, peer_node, dest_address):
    ...
    psbt = rbf_node.psbtbumpfee(txid=rbfid, options={"fee_rate": old_feerate + 200})
    finish_psbtbumpfee(psbt["psbt"])
...
def test_no_more_inputs_fails(self, rbf_node, dest_address):
    ...
    # In comparison to upstream, we need to be careful to create not a single
    # tx that is too big for Playchain's limit.
    unspent = rbf_node.listunspent ()
    rbfid = None
    while unspent:
      maxOutputs = 5
      current = unspent[:maxOutputs]
      unspent = unspent[maxOutputs:]
      txid = rbf_node.sendall(recipients=[rbf_node.getnewaddress()], options={"inputs": current})["txid"]
      if rbfid is None:
        rbfid = txid
