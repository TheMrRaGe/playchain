...
# total serialized size of the txouts is about 8k vbytes.
def gen_return_txouts():
    from .messages import CTxOut
    from .script import CScript, OP_RETURN
    txouts = [CTxOut(nValue=0, scriptPubKey=CScript([OP_RETURN, b'\x01'*8000]))]
    assert_equal(sum([len(txout.serialize()) for txout in txouts]), 8015)
    return txouts
...
def mine_large_block(test_framework, mini_wallet, node):
    ...
    create_lots_of_big_transactions(mini_wallet, node, fee, 12, txouts)
