...
class TestNode():
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        self.running = True
        self.log.debug("playchaind started, waiting for RPC to come up")
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'playchaind exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
        ...
        self._raise_assertion_error("Unable to connect to playchaind after {}s".format(self.rpc_timeout))
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'playchaind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'playchaind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    ...
    def __init__(self, binary, datadir):
        ...
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.playchaincli')
    ...
    def send_cli(self, command=None, *args, **kwargs):
        ...
        self.log.debug("Running playchain-cli {}".format(p_args[2:]))
