...
TMPDIR_PREFIX = "playchain_func_test_"
...
class BitcoinTestMetaClass(type):
    ...
    def __new__(cls, clsname, bases, dct):
        if not clsname in ['BitcoinTestFramework', 'NameTestFramework', 'PlaychainZmqTest']:
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave playchainds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop playchainds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use playchain-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_bitcoind = os.path.join(
            ...
            "src",
            "playchaind" + config["environment"]["EXEEXT"],
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "playchain-cli" + config["environment"]["EXEEXT"],
        )
        fname_playchainhash = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "playchain-hash" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "playchain-util" + config["environment"]["EXEEXT"],
        )
        self.options.bitcoind = os.getenv("BITCOIND", default=fname_bitcoind)
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=fname_bitcoincli)
        self.options.bitcoinutil = os.getenv("BITCOINUTIL", default=fname_bitcoinutil)
        powhash.playchainhash = os.getenv("PLAYCHAIN-HASH", default=fname_playchainhash)
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'playchaind', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'playchain-cli', self.options.bitcoincli) for v in versions]
