...
bool HeadersSyncState::ValidateAndProcessSingleHeader(const CBlockHeader& current)
{
    ...
    // adjustment maximum.
    //
    // TODO: Once we re-enable the difficulty transition checks for Playchain,
    // we need to keep track of the nBits and other retargeting data
    // for the last header received.
    /*
    if (!PermittedDifficultyTransition(m_consensus_params, next_height,
                m_last_header_received.pow.getBits(), current.pow.getBits())) {
...
bool HeadersSyncState::ValidateAndStoreRedownloadedHeader(const CBlockHeader& header)
{
    ...
    if (!m_redownloaded_headers.empty()) {
        previous_nBits = m_redownloaded_headers.back().pow.getBits();
    } else {
        previous_nBits = m_chain_start->nBits;
    }

    if (!PermittedDifficultyTransition(m_consensus_params, next_height,
                previous_nBits, header.pow.getBits())) {
