...
const char * const BITCOIN_CONF_FILENAME = "playchain.conf";
...
#else
    const char* pszModule = "playchain";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Playchain
    // macOS: ~/Library/Application Support/Playchain
    // Unix-like: ~/.playchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Playchain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Playchain";
#else
    // Unix-like
    return pathRet / ".playchain";
#endif
#endif
}
